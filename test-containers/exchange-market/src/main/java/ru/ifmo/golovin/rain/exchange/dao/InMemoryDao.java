...
public class InMemoryDao implements Dao {
    ...
    @Override
    public boolean acquireStocks(String companyName, int amount) {
        if (!this.stocks.containsKey(companyName)) {
            System.out.println("Unknown company" + companyName);
            return false;
        }
    ...
    @Override
    public boolean releaseStocks(String companyName, int amount) {
        if (!this.stocks.containsKey(companyName)) {
            System.out.println("Unknown company" + companyName);
            return false;
        }
        Company company = this.stocks.get(companyName);
        if (company.owned_stocks < amount) {
            System.out.println("Too many releasing stocks");
            return false;
        }
        company.amount_stocks += amount;
        company.owned_stocks -= amount;
    ...
    @Override
    public boolean resetStockPrice(String companyName, int price) {
        if (!this.stocks.containsKey(companyName)) {
            System.out.println("Unknown company" + companyName);
            return false;
        }
        Company company = this.stocks.get(companyName);
        company.price = price;
        System.out.println(String.format("New stock price (%s) for company '%s'", price, company.name));
        return true;
    }
}
